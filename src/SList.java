...
public class SList {
  ...
  public void squish() {
    ...
    SListNode lastMatch = head;
    SListNode check = head.next;
    
    while(check != null) {
    	if(check.item.equals(lastMatch.item)) {
    		lastMatch.next = check.next;
    		size--;
    	}
    	else 
    		lastMatch = tail = check;
  ...
  public void twin() {
    SListNode toDouble = head;
    
    while(toDouble != null) {
    	toDouble.next = new SListNode(toDouble.item, toDouble.next);
    	tail = toDouble.next;
    	toDouble = toDouble.next.next;
    	size++;
    }
  }
